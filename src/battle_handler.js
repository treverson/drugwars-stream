...
const helpers = require('../helpers/battle_helpers');
...
function executeBattleFirstStep(attacker, defender, cb) {
  ...
  if (defender.buildings.length > 0) {
    ...
    const thisround = {};
    const round_attackers = helpers.chooseNextAttackersByPriority(attacker.units);
    const round_defenders = helpers.chooseNextDefendersBuildingByPriority(defender.buildings);
    for (at = 0; at < defender.buildings.length; at++) {
      ...
      if (round_attackers && round_defenders) {
        ...
        for (u = 0; u < defender.buildings.length; u++) {
          if (
            defender.buildings[u].building === round_defenders.id ||
            defender.buildings[u].id === round_defenders.id
          ) {
      ...
      if (
        attacker.units &&
        attacker.units.length > 0 &&
        defender.buildings &&
        defender.buildings.length < 1
      ) {
        continueBattle(attacker, defender);
      } else {
        if (attacker.units.length < 1) console.log('attacker lost all units');
        else {
...
const battle_handler = {
  launchBattle(battle_key, cb) {
    ...
    db.query(query, [battle_key, battle_key], (err, attacker_result) => {
      ...
      db.query(query, [battle.defender, battle.defender], (err, defender_result) => {
        if (err) {
          console.log(err);
          return cb(null);
        }
        let [buildings, units] = defender_result;
        const defender = {};
        ...
        executeBattleFirstStep(attacker, defender, error => {
